            ...
            <form method="GET" action="{{ route('reports.purchases') }}">
                <div class="row align-items-end">
                    ...
                    <div class="col-md-3 d-flex gap-2 flex-wrap">
                        ...
                        <a href="{{ route('reports.purchases') }}" class="btn btn-outline-secondary"><i class="fas fa-times me-1"></i>Reset</a>
                        <a href="{{ route('reports.purchases.pdf') }}?{{ http_build_query(request()->query()) }}" target="_blank" class="btn btn-danger btn-sm"><i class="fas fa-file-pdf me-1"></i>PDF</a>
                        <a href="{{ route('reports.purchases.csv') }}?{{ http_build_query(request()->query()) }}" class="btn btn-success btn-sm"><i class="fas fa-file-excel me-1"></i>Excel</a>
                    </div>
                </div>
            </form>
        </div>
                ...
                <tbody>
                    @forelse($purchases as $purchase)
                        <tr>
                            ...
                            <td class="text-end">
                                {{ setting('currency_symbol') }}{{ format_number($purchase->total_amount ?? 0, 2) }}</td>
                            <td class="text-end">
                                {{ setting('currency_symbol') }}{{ format_number($purchase->discount_amount ?? 0, 2) }}</td>
                            <td class="text-end">
                                <strong>{{ setting('currency_symbol') }}{{ format_number($purchase->net_amount ?? 0, 2) }}</strong>
                            </td>
                            <td><small>—</small></td>
                        </tr>
                    @empty
